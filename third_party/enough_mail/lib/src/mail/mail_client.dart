...
class _IncomingImapClient extends _IncomingMailClient {
    ...
    if (trashMailbox == null || trashMailbox == selectedMailbox || expunge) {
      ...
      try {
        ...
        if (_imapClient.serverInfo.supportsMove) {
          ...
          // Cobalt Mail patch: see the matching note in _moveMessages above
          // — UID STORE must match a UID-based COPY.
          if (sequence.isUidSequence) {
            await _imapClient.uidStore(
              sequence,
              [MessageFlags.deleted],
  ...
  Future<MoveResult> _moveMessages(
    ...
    if (_imapClient.serverInfo.supports(ImapServerInfo.capabilityMove)) {
      ...
      // Cobalt Mail patch: the STORE call below must match the COPY call
      // above — a UID-based sequence needs UID STORE, not plain STORE
      // (which the server interprets as sequence numbers and rejects when
      // the UID value exceeds the mailbox's message count). Upstream bug:
      // https://github.com/Enough-Software/enough_mail (unconditional
      // _imapClient.store() here, unlike the correctly-branching uidCopy
      // above and the correctly-branching MailClient.store() elsewhere in
      // this same file). See docs/superpowers/... vendoring note.
      if (sequence.isUidSequence) {
        await _imapClient.uidStore(
          sequence,
          [MessageFlags.deleted],
